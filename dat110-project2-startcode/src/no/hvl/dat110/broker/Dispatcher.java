...
public class Dispatcher extends Stopable {
	...
	public void onConnect(ConnectMsg msg, Connection connection) {
		...
		// frivelig E
		if(storage.queue.containsKey(user)) {
			ClientSession session = storage.getSession(user);
			
			for(PublishMsg v : storage.queue.get(user)) {
				session.send(v);
			}
			storage.queue.remove(user);
		}
...
//		throw new UnsupportedOperationException(TODO.method());
	...
	public void onPublish(PublishMsg msg) {
		...
		for(String v : subscribers) {
			if(storage.getSession(v) == null ) {
				storage.addQueue(v, msg);
			}else {
				storage.getSession(v).send(msg);
			}
		}
